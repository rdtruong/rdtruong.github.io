...
_.typeOf=function(anything){
  return Object.prototype.toString.call(anything).toLowerCase().slice(8,-1);
}
...
_.first=function(arr, num){
    if(!Array.isArray(arr) || num<0){
        return [];
    }else if(typeof num==="undefined"|| num===isNaN){
        return arr[0];
...
_.unique=function (arr){
    var nodupe=[];
    for (var i=0; i<arr.length; i++){
       if(nodupe.indexOf(arr[i]) == -1)nodupe.push(arr[i]);
    }return nodupe;
}
/** _.map()
* Arguments:
...
_.map=function (collection,func){
 ...
 _.each(collection,function(element,index,array){
   arr.push(func(element,index,array));
 }); return arr; 
};
...
_.pluck=function(arr, prop){
   var newarr=[];
  _.map(arr,function(obj,index,array){
     newarr.push(array[index][prop]);
  }); return newarr;
};
/** _.contains()
* Arguments:
